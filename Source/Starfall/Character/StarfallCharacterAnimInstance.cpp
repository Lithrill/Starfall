...
void UStarfallCharacterAnimInstance::NativeUpdateAnimation(float DeltaTime)
{
	...
	if (bWeaponEquipped && EquippedWeapon && EquippedWeapon->GetWeaponMesh() && StarfallCharacter->GetMesh())
	{
		...
		StarfallCharacter->GetMesh()->TransformToBoneSpace(FName("R_Wrist_Jnt"), LeftHandTransform.GetLocation(), FRotator::ZeroRotator, OutPosition, OutRotation);
		...
		if (StarfallCharacter->IsLocallyControlled())
		{
			...
			FTransform RightHandTransform = EquippedWeapon->GetWeaponMesh()->GetSocketTransform(FName("R_Wrist_Jnt"), ERelativeTransformSpace::RTS_World);
